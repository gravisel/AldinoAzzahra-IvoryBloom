...
document.getElementById("save-date").addEventListener("click", () => {
  const event = {
    title: "Pernikahan Aldino Azzahra",
    description: "Save the date – Wedding Day 💍",
    location: "Gedung / Alamat Acara",
    start: "20260208T090000", // YYYYMMDDTHHMMSS
    end: "20260208T180000"
  };
...
btn.addEventListener("click", prepareRsvpData);
...
async function prepareRsvpData() {
  ...
  function showRsvpOverlay(message, type = "success", duration = 2500) {
    const overlay = document.getElementById("rsvp-status-text");
    const box = document.getElementById("rsvp-status-box");

    box.textContent = message;

    // reset warna
    box.classList.remove("text-white", "text-red-400", "bg-green-700", "bg-red-700", "bg-white", "bg-black");

    if (type === "error") {
      box.classList.add("text-white", "bg-red-700");
    } else {
      box.classList.add("text-white", "bg-green-700");
    }
  ...
  // VALIDASI
  if (!name) {
    showRsvpOverlay("Nama wajib diisi", "error");
    return;
  }

  if (!statusSelect.value) {
    showRsvpOverlay("Pilih status kehadiran", "error");
    return;
  }
  ...
  try {
    ...
    showRsvpOverlay("Konfirmasi berhasil dikirim ✅", "success");

    setTimeout(() => {
      showRsvpOverlay("Terima kasih atas konfirmasinya 💐", "success", 2000);
    }, 3000);
    ...
    showRsvpOverlay("Gagal mengirim ❌. Coba lagi.", "error");
  } finally {
...
btnRSVP.addEventListener("click", () => {
  ...
  setTimeout(() => {
    loading.classList.add("hidden");
    showRsvpOverlay("Mengalihkan ke WhatsApp…", "success");
    window.open(url, "_blank");
